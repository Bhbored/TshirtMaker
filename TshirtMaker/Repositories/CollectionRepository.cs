...
namespace TshirtMaker.Repositories
{
    ...
    {
        public CollectionRepository(Supabase.Client supabaseClient, string supabaseUrl, string supabaseKey) 
            : base(supabaseClient, "collections", supabaseUrl, supabaseKey)
        {
        }
        ...
        {
            var offset = (pageNumber - 1) * pageSize;
            var response = await _httpClient.GetAsync($"/rest/v1/{_tableName}?copied_design_id=eq.{copiedDesignId}&order=created_at.desc&offset={offset}&limit={pageSize}");
            response.EnsureSuccessStatusCode();
        ...
        {
            var response = await _httpClient.GetAsync($"/rest/v1/{_tableName}?user_id=eq.{userId}&copied_design_id=eq.{copiedDesignId}");
            response.EnsureSuccessStatusCode();
